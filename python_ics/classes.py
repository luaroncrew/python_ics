...
class Event:
    def __init__(
        ...
        self.notes = self._is_valid_description(description)
    ...
    def _is_valid_dt(self, dt_param):
    ...
    def _is_valid_description(self, notes):
        if notes is not None:
            if not isinstance(notes, str):
                raise TypeError('notes can be str or None type')
        if len(notes) > 75:
            raise ValueError('title cannot be longer than 75 symbols')
        return notes

    def __str__(self):
        return f'{self.title}, {self.location}, {self.notes}, {self.dt_end}, {self.dt_start}'
...
class Calendar:
    ...
    def get_execution_string(self):
        ...
        execution_string += self.setup.string()
